module.exports = (sequelize, DataTypes) => {
  ...
  User.associate = db => {
    db.User.hasMany(db.Post, { as: 'Post' }); // as 를 통해 조합이 같은 테이블과 구별. 꼭 적어줘야함.
    ...
    db.User.belongsToMany(db.User, { through: 'Follow', as: 'Followers' }); // 같은 테이블 끼리 다대다관계이면 구별을 위해 as로 구별
    db.User.belongsToMany(db.User, { through: 'Follow', as: 'Follwings' });
  };
